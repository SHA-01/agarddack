...
// @author       You
// @match        http://agar.co.kr/
// @grant        none
// @webRequest   [{"selector":"assets/js/main_out.js","action":"cancel"}]
// ==/UserScript==
(function(wHandle, wjQuery) {
    var CONNECTION_URL = "agar.co.kr:443", // Default Connection
    ...
    var multiboxs = function() {
        var doubleSocket = new WebSocket("ws://" + CONNECTION_URL);
        var leftPos1, topPos1, botWS, posX1, posY1;


        doubleSocket.binaryType = "arraybuffer";
        doubleSocket.onopen = function() {
            var msg = getBinaryData(5);
            msg.setUint8(0, 254);
            ...
            let name = "bot";
            buf = getBinaryData(1 + 2 * name.length);
            buf.setUint8(0, 0);
            for (var i = 0; i < name.length; ++i) buf.setUint16(1 + 2 * i, name.charCodeAt(i), true);
            this.send(buf);

        }
        doubleSocket.onmessage = function(e) {
            var Binary = new DataView(e.data);
            var offset = 0;

            240 == Binary.getUint8(offset) && (offset += 5);
            switch (Binary.getUint8(offset++)) {
                case 16: // update nodes
                    updateNodes(Binary, offset, false);
                    break;
                case 17: // update position
                    posX = Binary.getFloat32(offset, true);
                    offset += 4;
                    posY = Binary.getFloat32(offset, true);
                    offset += 4;
                    posSize = Binary.getFloat32(offset, true);
                    offset += 4;
                    break;
                case 20: // clear nodes
                    playerCells = [];
                    nodesOnScreen = [];
                case 32: // add node
                    nodesOnScreen.push(Binary.getUint32(offset, true));
                    offset += 4;
                    break;
                case 64:
                    leftPos1 = Binary.getFloat64(1, true) || leftPos1
                    topPos1 = Binary.getFloat64(9, true) || topPos1
                    posSize = 1;
                    if (0 == playerCell1.length) {
                        viewZoom1 = posSize;
                    }
        ...
        function getBinaryData(e) {
            return new DataView(new ArrayBuffer(e));
        }

        return {
            split: function() {
                if (botWS == undefined) return;
                let buf = getBinaryData(1)
                buf.setUint8(0, 17);
                botWS.send(buf, true);
            },
            eject: function() {
            ...
            mouseMoveTo(x, y) {
                if (botWS == undefined) return;
                let buf = getBinaryData(21)
                buf.setUint8(0, 16);
                buf.setFloat64(1, x, true);
                buf.setFloat64(9, y, true);
                buf.setUint32(17, 0, true);
                botWS.send(buf, true);
            },
            spawn: function(name) {
                if (botWS == undefined || name == undefined) return;
                console.log(name)
                let buf = getBinaryData(1 + 2 * name.length);
                buf.setUint8(0, 0);
                for (var i = 0; i < name.length; ++i) buf.setUint16(1 + 2 * i, name.charCodeAt(i), true);
                botWS.send(buf);
            },
    ...
    function DrawMini() {
        ...
        const unit = 6;
        ctx.fillStyle = (showDarkTheme) ? "#FFFFFF" : "#000000"
        ...
        ctx.stroke();
        ctx.closePath();
        var Celllist = TargetMain ? playerCells : playerCell1;
        var PLAYLENG = Celllist.length;
        for (var i = 0; i < PLAYLENG; i++) {
            var Cellox = (Celllist[i].ox - (rightPos - ((rightPos - leftPos) / 2))) / (rightPos - leftPos) * Toucha;
            var Celloy = (Celllist[i].oy - (bottomPos - ((rightPos - leftPos) / 2))) / (bottomPos - topPos) * Toucha;
            //console.log(rightPos, leftPos, bottomPos, topPos)
            //console.log(Celllist[i].ox - (rightPos - 12500), Celllist[i].oy - (bottomPos - 12500))
            var x = TouWidth + (Toucha / 2) + Cellox;
            var y = Touheight + (Toucha / 2) + Celloy;
            var Sizehalf = Math.max(2, Celllist[i].size / (Toucha / 4));
            var showAlpha = 0.4;
            if (showColor) {
                if (showDarkTheme) {
            ...
            ctx.fillStyle = (showDarkTheme) ? "#9AFF35" : "#88D140";
            ctx.beginPath();
            ctx.arc(x, y, Math.max(5, Sizehalf), 0, 2 * Math.PI);
            ctx.stroke();
            ctx.fill();
            ctx.closePath();
        }
        ctx.strokeStyle = (showDarkTheme) ? "#FFFFFF" : "#000000" //white : dark
        ctx.lineWidth = 3;
        ctx.globalAlpha = 0.4;
        for (var i = 0; i < unit; i++) {
            for (var ii = 0; ii < unit; ii++) {
                ctx.rect(TouWidth + (Toucha / unit * i), Touheight + (Toucha / unit * ii), Toucha / unit, Toucha / unit);
            }
        }
        for (var i = 0; i < unit; i++) {
            for (var ii = 0; ii < unit; ii++) {
                ctx.font = "bold 14px Ubuntu, sans-serif";
                ctx.fillText((ii + 1).toString() + (i + 1).toString(), TouWidth + (Toucha / (unit * 2) * (2 * i + 1)), Touheight + (Toucha / (unit * 2) * (2 * ii + 1)) + 10)
            }
        }
        ctx.stroke();
        ctx.closePath();
            ...
            switch (event.keyCode) {
                ...
                case 87: // W
                    if ((!wPressed) && (!isTyping)) {
                        ...
                        // eMacro = setInterval(function () {
                        //     if (wPressed) {
                        //         sendUint8(21);
                        //     } else clearInterval(eMacro);
                        // }, 32);
                    }
                    break;
                    /* Planned At PR_Agark_agarMacro __20191111 to Native inject-from agark.js */
                case 81: // Q
    ...
    function handleWheel(event) {
        ...
        1 > zoom && (zoom = 1);
        viewZoom = TargetMain ? viewZoom : viewZoom1;
    ...
    function buildQTree() {
        if (.4 > viewZoom) qTree = null;
            ...
            for (var i = 0; i < nodelist.length; i++) {
                var node = nodelist[i];
                if (!node.prepareData && 20 < node.size * viewZoom) {
            ...
            viewZoom = TargetMain ? viewZoom : viewZoom1
            var VX = TargetMain ? nodeX : nodeX1;
            var VY = TargetMain ? nodeX1 : nodeY1
            for (i = 0; i < nodelist.length; i++) {
                node = nodelist[i];
                if (!(20 >= node.size * viewZoom)) {
                    for (a = 0; a < node.points.length; ++a) {
                        b = node.points[a].x;
                        c = node.points[a].y;
                        b < VX - canvasWidth / 2 / viewZoom || c < VY - canvasHeight / 2 / viewZoom || b > VX + canvasWidth / 2 / viewZoom || c > VY + canvasHeight / 2 / viewZoom || qTree.insert(node.points[a]);
                    }
                }
            }
        }
    ...
    function mouseCoordinateChange() {
        ...
        X1 = (rawMouseX - canvasWidth / 2) / viewZoom1 + nodeX1;
        Y1 = (rawMouseY - canvasHeight / 2) / viewZoom1 + nodeY1
    }

    function hideOverlays() {
        hasOverlay = false;
        wjQuery("#adsBottom").hide();
        wjQuery("#canvas").slideDown(1000);
    }

    function showOverlays(arg) {
        wjQuery("#canvas").slideUp(1000);
    ...
    function updateNodes(view, offset, isMain) {
        timestamp = +new Date;
        ...
        for (var i = 0;;) {
            ...
            var size, posY, posX = view.getInt32(offset, true); //console.log(offset)
            offset += 4;
            ...
            if (isMain == true) {
                ...
                if (-1 != nodesOnScreen.indexOf(nodeid) && -1 == playerCell1.indexOf(node)) {
                    nodeX1 = node.x;
                    nodeY1 = node.y;
                    // if (playerColor == null) {
                    //     playerColor = node.color;
                    // }
                    //console.log(playerCell1)
                    playerCell1.push(node);
                    if (1 == playerCell1.length) {
                        nodeX1 = node.x;
                        nodeY1 = node.y;
                    }
                }
        ...
        var Celllist = TargetMain ? playerCells : playerCell1;
        ua && 0 == Celllist.length && showOverlays(false)
    }

    function hideOverlays_() {
        wjQuery("#canvas").slideDown(1000);
    }

    function showOverlays_() {
        wjQuery("#canvas").slideUp(1000);
    }

    function sendMouseMove() {
        var msg;
        if (wsIsOpen() && TargetMain == true) {
        ...
        } else if (TargetMain == false) {
            multibox1.mouseMoveTo(X1, Y1)
                //console.log(X1, Y1)
        }
    }
    ...
    function calcViewZoom2() {
        if (0 != playerCell1.length) {
            for (var newViewZoom = 0, i = 0; i < playerCell1.length; i++) newViewZoom += playerCell1[i].size;
            newViewZoom = Math.pow(Math.min(64 / newViewZoom, 1), .4) * viewRange();
            viewZoom1 = (9 * viewZoom1 + newViewZoom) / 10;
        }
    }
    ...
    function drawGameScene() {
        ...
        if (TargetMain == false) {
            if (0 < playerCell1.length) {
                ...
                for (var d = 0; d < playerCell1.length; d++) {
                    playerCell1[d].updatePos();
                    //console.log(playerCell1[d].destroyed)
                    a += playerCell1[d].x / playerCell1.length;
                    c += playerCell1[d].y / playerCell1.length;
                    if (playerCell1[d].destroyed == true) {
                        playerCell1.splice(d, d + 1);
                    }
                }
                posX = a;
                posY = c;
                ...
                nodeX1 = (29 * nodeX1 + posX) / 30;
                nodeY1 = (29 * nodeY1 + posY) / 30;
                viewZoom1 = (9 * viewZoom1 + posSize * viewRange()) / 10;
            }
        } else {
            if (0 < playerCells.length) {
                calcViewZoom();
                ...
                for (var d = 0; d < playerCells.length; d++) {
                    ...
                    if (playerCells[d].destroyed == true) {
                        playerCells.splice(d, d + 1)
                    }
                }
                posX = a;
                posY = c;
        ...
        if (drawLine) {
            ...
            ctx.beginPath();
            var Celllist = TargetMain ? playerCells : playerCell1
            for (d = 0; d < Celllist.length; d++) {
                ctx.moveTo(Celllist[d].x, Celllist[d].y);
    ...
    function drawGrid() {
        ...
        for (var c = -.5 + (-VX + a / 2) % 50; c < a; c += 50) {
            ctx.moveTo(c, 0);
            ctx.lineTo(c, b);
        }
        ctx.stroke();
        ctx.beginPath();
        for (c = -.5 + (-VY + b / 2) % 50; c < b; c += 50) {
            ctx.moveTo(0, c);
            ctx.lineTo(a, c);
        }
        ctx.stroke()
    ...
    function calcUserScore() {
        var Celllist = TargetMain ? playerCells : playerCell1
        for (var score = 0, i = 0; i < Celllist.length; i++) score += Celllist[i].nSize * Celllist[i].nSize;
        return score
    }
    ...
    function drawLeaderBoard() {
        ...
        if (drawTeam || 0 != leaderBoard.length)
            if (drawTeam || showName) {
                ...
                ctx.fillStyle = "#FFFFFF";
                var c = "순위판";
                ctx.font = "30px NanumSquare";
                ctx.fillText(c, 100 - ctx.measureText(c).width * 0.5, 40);
                var b, l;
                if (!drawTeam) {
                    for (ctx.font = "20px NanumSquare", b = 0, l = leaderBoard.length; b < l; ++b) {
                        ...
                        var Celllist = TargetMain ? playerCells : playerCell1;
                        if (me) Celllist[0].name && (c = Celllist[0].name);
    ...
    function Octa(Xcenter, Ycenter, size, numberOfSides) {
    ...
    var nCanvas, ctx, mainCanvas, lbCanvas, chatCanvas, canvasWidth, canvasHeight, multibox1, qTree = null,
        ws = null,
        TargetMain = true,
        nodeX = 0,
        nodeY = 0,
    ...
    wHandle.setNick = function(arg) {
        ...
        userNickName = arg;
        sendNickName();
        if (hasMultibox == false) {
            multibox1 = new multiboxs()
            hasMultibox = true;
        }
        userScore = 0
    };
    ...
    Cell.prototype = {
        ...
        destroy: function() {
            ...
            tmp = Celllist.indexOf(this);
            if (-1 != tmp) {
                ua = true;
                Celllist.splice(tmp, 1);
            }
        ...
        shouldRender: function() {
            if (0 == this.id) {
                ...
                return true //!(this.x + this.size + 40 < nodeX - canvasWidth / 2 / viewZoom || this.y + this.size + 40 < nodeY - canvasHeight / 2 / viewZoom || this.x - this.size - 40 > nodeX + canvasWidth / 2 / viewZoom || this.y - this.size - 40 > nodeY + canvasHeight / 2 / viewZoom);
            }
        },
            ...
            if (this.shouldRender()) {
                ...
                if (b) {
                    ...
                    if (this.name.length == 0) {
                        ...
                        //Octa(this.x, this.y, (this.size - lw * 0.5 + 5), 3 + parseInt(this.size / 10)); // Math.max(8,8+parseInt(this.size/200))
                        ...
                        ctx.fill()
                        ctx.stroke()
                        if (-1 != playerCells.indexOf(this) && TargetMain == true) {
                            // ctx.stroke()
                            ctx.beginPath()
                            ctx.lineWidth = 1 / viewZoom * 2;
                            ctx.moveTo(this.x, this.y)
                            ctx.lineTo(X, Y);

                        }
                        if (-1 != playerCell1.indexOf(this) && TargetMain == false) {
                            // ctx.stroke()
                            ctx.beginPath()
                            ctx.lineWidth = 1 / viewZoom * 2;
                            ctx.moveTo(this.x, this.y)
                ...
                if (showSkin && skinName != '') {
                    ...
                    // var skinsUrl = document.querySelector("#Skin_url").value
                    // console.log(skinsUrl);
                    // if (skinsUrl.length > 1) {
                    //     skins[skinName].src = skinsUrl;
                    // }
                ...
                if (0 != this.id) {
                    ...
                    //draw mass
                    var Celllist = TargetMain ? playerCells : playerCell1;
                    if (this.sizeCache != null && TargetMain == true && showMass && (c || 0 == Celllist.length && (!this.isVirus || this.isAgitated) && 20 < this.size)) {
                        var m = ~~(this.size * this.size * 0.01);
                        c = this.sizeCache;
                        c.setValue(m);
                        c.setScale(ratio);
