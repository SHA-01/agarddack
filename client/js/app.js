...
AppClass.prototype.updateInputs = function () {
    ...
    $('#playBtn').click(function () {
        ...
        for (var i = 0; i < prevent_words.length; i++) {
            if (inputed.indexOf(prevent_words[i]) !== -1) {
                alert('湲덉��닿� �ы븿�섏뼱 �덉뒿�덈떎.[' + prevent_words[i] + ']');
                return false;
            }
        }
...
var showMsgModal = function (msg, callback) {
    ...
    if ($('#alertModal').length == 0 || !$('#alertModal').modal) {
        ...
        var modalTittle = '<i class="fa fa-info"></i> �뚮┝';

        if (msg.substr(0, loadingMsg.length) == loadingMsg) {
            var modalId = msg.substr(loadingMsg.length);
            msg = '<div  calss="text-center" id="' + modalId + '"><i class="fa fa-spinner fa-spin"></i> �좎떆留� 湲곕떎�ㅼ＜�몄슂.</div>';
            modalTittle = '濡쒕뵫以�';
        }
